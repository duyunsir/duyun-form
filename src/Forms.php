    ...
    protected function getform($data = [], array $config = [])
    {
        ...
        if ($form = $Model::getOne($_where)) {
            ...
            if (isset($config['ext_id']) && $config['ext_id']) {
                ...
                if ($extfields = \app\backstage\model\Extendfield::getList($_where,['sort' => 'desc','id' => 'asc'])) {
                    // $extgroups = [];
                ...
                if ($extfields = \app\backstage\model\Extendfield::where($_where)->order('sort desc,id asc')->select()) {
                    // $extgroups = [];
                    foreach ($extfields as $key => $extfield) {
                        // 字段配置
                        $tmp = [
