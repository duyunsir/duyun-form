...
<div class="input-group date">
    ...
    <input style="margin-top: 0;" type="text" class="form-control help-block m-b-none" name="{$_field.field}@{$_field.condition}" id="{$_ns}" value="{$_field.value}" {if condition='$_field.config.disabled eq 1'} disabled {/if} {if condition='$_field.config.readonly eq 1'} readonly {/if}{if condition='$_field.config.required eq 1'} required {/if}>
</div>
...
<script>
    $(function () {
        ...
        $('#{$_ns}').daterangepicker(picker, function (start, end, label) {
            $('#{$_ns}').val(start.format('YYYY-MM-DD') + ' 到 ' + end.format('YYYY-MM-DD'));
            console.log('New date range selected: ' + start.format('YYYY-MM-DD') + ' to ' + end.format('YYYY-MM-DD') + ' (predefined range: ' + label + ')');
        });
        $('#{$_ns}').on('cancel.daterangepicker', function(ev, picker) {
            $(this).val('');
        });
    })
</script>