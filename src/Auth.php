 ...
 * 3，一个用户可以属于多个用户组(think_auth_access表 定义了用户所属用户组)。我们需要设置每个用户组拥有哪些规则(think_auth_group 定义了用户组权限)
 *
 * 4，支持规则表达式。
...
class Auth
{
    ...
    public function getGroups($uid)
    {
        ...
        $user_groups = Db::name($this->_config['auth_access'])
            ->alias('a')
            ->where('a.uid', $uid)
            ->where('g.status', 1)
